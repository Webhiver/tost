...
const Header = () => {
    ...
    const firmwareUpdateAvailable = useContextSelector(LocalContext, c => c.firmwareUpdateAvailable);
    ...
    return (
        <header className="flex justify-between items-center gap-4 px-6 py-4">
            ...
            {(!isPairing && firmwareUpdateAvailable) && <TbCloudDownload className="text-sky-600 size-6 dark:text-sky-400 cursor-pointer animate-bounce" onClick={onOpenPanel("updates")}/>}
