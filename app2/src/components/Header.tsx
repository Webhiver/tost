...
const Header = () => {
    ...
    return (
        <header className="flex justify-between items-center gap-4 px-6 py-4">
            ...
            <TbCloudDownload className="text-sky-600 size-6 dark:text-sky-400 cursor-pointer animate-bounce" onClick={onOpenPanel("updates")}/>
            {type === "host" ? <MdOutlineMenuOpen className="fill-slate-500 size-8 dark:fill-slate-200 cursor-pointer" onClick={onOpenPanel("main")}/> : null}
        </header>
    )
}
